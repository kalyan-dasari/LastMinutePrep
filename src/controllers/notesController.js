...
router.get("/notes/:id/share", async (req, res) => {
  const note = await get(
    `SELECT n.id, n.title, u.name AS contributor_name
       FROM notes n
       JOIN users u ON u.id = n.user_id
      WHERE n.id = ?`,
    [req.params.id]
  ...
  const shareUrl = `${req.protocol}://${req.get("host")}/notes/${note.id}`;
  res.render("share-note", {
...
router.get("/search/suggestions", async (req, res) => {
  ...
  const subjects = await all(
    `SELECT DISTINCT subject AS text, COUNT(*) AS count
       FROM notes
      WHERE subject LIKE ?
      GROUP BY subject
      ORDER BY count DESC
  ...
  const branches = await all(
    `SELECT DISTINCT branch AS text, COUNT(*) AS count
       FROM notes
      WHERE branch LIKE ?
      GROUP BY branch
      ORDER BY count DESC
